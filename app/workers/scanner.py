...
class Scanner:
    def __init__(self, rpc_url: str):
        self.rpc_url = rpc_url
        self.web3: AsyncWeb3Adapter | None = None

    async def init(self):
        self.web3 = await AsyncWeb3Adapter.get_instance(self.rpc_url)
        await self.web3.ensure_connection()

    async def scan_latest_block(self):
        block = await self.web3.get_latest_block()
        print("Latest block:", block)
        return block


async def main():
    scanner = Scanner("https://polygon-rpc.com")
    await scanner.init()
    await scanner.scan_latest_block()
